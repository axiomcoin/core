Gitian building
================
*Setup instructions for a Gitian build of Nirvana Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nirvana
...
- [Building Nirvana Core](#building-nirvana-core)
...
Clone the git repositories for nirvana and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nirvana-fnd/nirvana --recursive
...
```bash
sudo cp /home/debian/nirvana/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the nirvana repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Nirvana Core
----------------

To build Nirvana Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the nirvana repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nirvana/.git/
...
For example:
```bash
URL=https://github.com/laanwj/nirvana.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit nirvana=${COMMIT} --url nirvana=${URL} ../nirvana/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nirvana=${COMMIT} --url nirvana=${URL} ../nirvana/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nirvana=${COMMIT} --url nirvana=${URL} ../nirvana/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-nirvana:
./bin/gbuild --commit nirvana=${COMMIT},cpp-eth-nirvana=${ETHCOMMIT} --url nirvana=${URL},cpp-eth-nirvana=${ETHURL} ../nirvana/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the nirvana git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../nirvana/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/nirvana-fnd/nirvana-detached-sigs.git

BTCPATH=/some/root/path/nirvana
SIGPATH=/some/root/path/nirvana-detached-sigs

./bin/gbuild --url nirvana=${BTCPATH},signature=${SIGPATH} ../nirvana/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nirvana-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nirvana-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/nirvana-osx-build.assert
```
...
request. You can also mail the files to Jordan Earls (earlz@nirvana.network) and he will commit them.
