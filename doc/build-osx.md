...
1. Clone the Nirvana Core source code:
    ```shell
    git clone --recursive https://github.com/nirvana-fnd/nirvana.git
    cd nirvana
    ```

2.  Build Nirvana Core:

    Configure and build the headless Nirvana Core binaries as well as the GUI (if Qt is found).
...
## Running
Nirvana Core is now available at `./src/nirvanad`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Nirvana"

touch "/Users/${USER}/Library/Application Support/Nirvana/nirvana.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Nirvana/nirvana.conf"
```

The first time you run nirvanad, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Nirvana/debug.log
```

## Other commands:
```shell
./src/nirvanad -daemon      # Starts the nirvana daemon.
./src/nirvana-cli --help    # Outputs a list of command-line options.
./src/nirvana-cli help      # Outputs a list of RPC commands when the daemon is running.
```
