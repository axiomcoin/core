...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setNirvanaSchedule(nirvanaDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = nirvanaDGP.getBlockSize(nHeight);
    minGasPrice = nirvanaDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = nirvanaDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    NirvanaTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractNirvanaTX resultConverter;
    if(!convert.extractionNirvanaTransactions(resultConverter)){
    ...
    for(NirvanaTransaction nirvanaTransaction : nirvanaTransactions){
        txGas += nirvanaTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + nirvanaTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(nirvanaTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, nirvanaTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            nirvanaDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = nirvanaDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                nirvanaDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                nirvanaDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            nirvanaDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            nirvanaDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    NirvanaDelegation nirvanaDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                nirvanaDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = nirvanaDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    nirvanaDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    nirvanaDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                nirvanaDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(nirvanaDelegations.GetDelegation(address, delegation) && NirvanaDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "nirvanastake";
...
void StakeNirvanas(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
