...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
    ...
    genesis.hashStateRoot = uint256(h256Touint(dev::h256("e965ffd002cd6ad0e2dc402b8044de833e06b23127ea8c3d80aec91410771495"))); // nirvana
    genesis.hashUTXORoot = uint256(h256Touint(dev::sha3(dev::rlp("")))); // nirvana
    return genesis;
}
...
public:
    CMainParams() {
        strNetworkID = "main";
        consensus.nSubsidyHalvingInterval = 2628000; // nirvana halving every 5 years
        ...
        consensus.nMinimumChainWork = uint256S("0x00"); // nirvana
        ...
        vSeeds.emplace_back("mainnet.nirvana.network"); // Nirvana mainnet
        vSeeds.emplace_back("mainnet.nirvana.seeds.vidaru.org"); // Nirvana mainnet
...
public:
    CTestNetParams() {
        strNetworkID = "test";
        consensus.nSubsidyHalvingInterval = 985500; // nirvana halving every 4 years
        consensus.nSubsidyHalvingInterval = 2628000; // nirvana halving every 5 years
        ...
        vSeeds.emplace_back("testnet.nirvana.network"); // Nirvana testnet
        vSeeds.emplace_back("testnet.nirvana.seeds.vidaru.org"); // Nirvana testnet
...
public:
    ...
    : CRegTestParams(args)
    {
        ...
        // NIRVANA have 500 blocks of maturity, increased values for regtest in unit tests in order to correspond with it
...
{
    return dev::eth::Network::nirvanaMainNetwork;
}
