...
const char * const BITCOIN_CONF_FILENAME = "nirvana.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nirvana
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nirvana
    // Mac: ~/Library/Application Support/Nirvana
    // Unix: ~/.nirvana
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nirvana";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nirvana";
#else
    // Unix
    return pathRet / ".nirvana";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Nirvana Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Nirvana Core Developers";
    }
