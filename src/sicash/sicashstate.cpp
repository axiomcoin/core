...
#include <nirvana/nirvanastate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

NirvanaState::NirvanaState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = NirvanaState::openDB(_path + "/nirvanaDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

NirvanaState::NirvanaState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute NirvanaState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, NirvanaTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createNirvanaAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            nirvana::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, NirvanaTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, NirvanaTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> NirvanaState::vins() const // temp
{
...
void NirvanaState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* NirvanaState::vin(dev::Address const& _a) const
{
    return const_cast<NirvanaState*>(this)->vin(_a);
}

Vin* NirvanaState::vin(dev::Address const& _addr)
{
...
// void NirvanaState::commit(CommitBehaviour _commitBehaviour)
// {
...
void NirvanaState::kill(dev::Address _addr)
{
...
void NirvanaState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void NirvanaState::deleteAccounts(std::set<dev::Address>& addrs){
...
void NirvanaState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void NirvanaState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void NirvanaState::validateTransfersWithChangeLog(){
...
void NirvanaState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!NirvanaState::addressInUse(delegationsAddress)){
        NirvanaState::createContract(delegationsAddress);
        NirvanaState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
