...
void NirvanaDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool NirvanaDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule NirvanaDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t NirvanaDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t NirvanaDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t NirvanaDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t NirvanaDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool NirvanaDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void NirvanaDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void NirvanaDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void NirvanaDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void NirvanaDGP::createParamsInstance(){
...
dev::Address NirvanaDGP::getAddressForBlock(unsigned int blockHeight){
...
void NirvanaDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NirvanaDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NirvanaDGP::parseStorageOneUint64(uint64_t& value){
...
void NirvanaDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule NirvanaDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
