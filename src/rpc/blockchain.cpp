...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // nirvana
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // nirvana

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // nirvana
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // nirvana
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // nirvana
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // nirvana
...
UniValue transactionReceiptToJSON(const NirvanaTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination nirvanaSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(nirvanaSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&nirvanaSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The nirvana address string"},
                },
                RPCResult{
            "{\n"
    ...
    NirvanaDelegation nirvanaDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!nirvanaDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = nirvanaDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The nirvana address string for staker"},
                },
                RPCResult{
            "[{\n"
    ...
    if(!nirvanaDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = nirvanaDelegation.DelegationsFromEvents(events);
...
UniValue gettxout(const JSONRPCRequest& request)
{
            ...
            "     \"addresses\" : [          (array of string) array of nirvana addresses\n"
            "        \"address\"     (string) nirvana address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
