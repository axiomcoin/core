...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // nirvana
    uint256 hashUTXORoot; // nirvana
    ...
    void SetNull()
    {
        ...
        nBits          = 0;
        nNonce         = 0;
        hashStateRoot  = uint256(); // nirvana
        hashUTXORoot   = uint256(); // nirvana
    ...
    explicit CBlockIndex(const CBlockHeader& block)
    {
        ...
        nNonce         = block.nNonce;
        nMoneySupply   = 0;
        hashStateRoot  = block.hashStateRoot; // nirvana
        hashUTXORoot   = block.hashUTXORoot; // nirvana
        nStakeModifier = uint256();
        hashProof = uint256(); 
        prevoutStake   = block.prevoutStake; // nirvana
        vchBlockSigDlgt    = block.vchBlockSigDlgt; // nirvana
    }
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // nirvana
        block.hashUTXORoot   = hashUTXORoot; // nirvana
    ...
    bool IsProofOfWork() const // nirvana
    {
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // nirvana
        READWRITE(hashUTXORoot); // nirvana
        READWRITE(nStakeModifier);
        READWRITE(prevoutStake);
        READWRITE(hashProof);
        READWRITE(vchBlockSigDlgt); // nirvana
    }
