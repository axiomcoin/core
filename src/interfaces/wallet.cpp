...
public:
    ...
    bool isUnspentAddress(const std::string &nirvanaAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == nirvanaAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_nirvanaDelegation.ExistDelegationContract() ? m_nirvanaDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_nirvanaDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_nirvanaDelegation.ExistDelegationContract() ? m_nirvanaDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_nirvanaDelegation.VerifyDelegation(address, delegation);
    ...
    NirvanaDelegation m_nirvanaDelegation;
};
