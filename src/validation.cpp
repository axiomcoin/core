...
#if defined(NDEBUG)
# error "Nirvana cannot be compiled without assertions."
#endif
...
std::unique_ptr<NirvanaState> globalState;
...
bool fAddressIndex = false; // nirvana
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        NirvanaDGP nirvanaDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = nirvanaDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        NirvanaTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractNirvanaTX resultConverter;
        if(!converter.extractionNirvanaTransactions(resultConverter)){
        ...
        std::vector<NirvanaTransaction> nirvanaTransactions = resultConverter.first;
        std::vector<EthTransactionParams> nirvanaETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(NirvanaTransaction nirvanaTransaction : nirvanaTransactions){
            sumGas += nirvanaTransaction.gas() * nirvanaTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, nirvanaTransaction.gasPrice());
            } else {
                txMinGasPrice = nirvanaTransaction.gasPrice();
            }
            VersionVM v = nirvanaTransaction.getVersion();
            if(v.format!=0)
            ...
            if(nirvanaTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(nirvanaTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(nirvanaTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += nirvanaTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)nirvanaTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NIRVANA additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // nirvana
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // nirvana
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    NirvanaTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = NirvanaTransaction(0, 1, dev::u256(gasLimit), opcode, dev::u256(0));
    }
    else
    {
        callTransaction = NirvanaTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<NirvanaTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path nirvanaDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(nirvanaDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(nirvanaDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(NirvanaTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, NirvanaTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool NirvanaTxConverter::extractionNirvanaTransactions(ExtractNirvanaTX& nirvanatx){
    ...
    std::vector<NirvanaTransaction> resultTX;
    ...
    nirvanatx = std::make_pair(resultTX, resultETP);
    return true;
}

bool NirvanaTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool NirvanaTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
NirvanaTransaction NirvanaTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    NirvanaTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = NirvanaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = NirvanaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t NirvanaTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // nirvana
    NirvanaDGP nirvanaDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setNirvanaSchedule(nirvanaDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = nirvanaDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = nirvanaDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // nirvana
...
///////////////////////////////////////////////////////////////////////////////////////// nirvana
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            NirvanaTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractNirvanaTX resultConvertNirvanaTX;
            if(!convert.extractionNirvanaTransactions(resultConvertNirvanaTX)){
            ...
            if(!CheckMinGasPrice(resultConvertNirvanaTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertNirvanaTX.first, blockGasLimit, pindex->pprev);
            ...
            for(NirvanaTransaction& qtx : resultConvertNirvanaTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertNirvanaTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertNirvanaTX.first[k].from(),
                        resultConvertNirvanaTX.first[k].to(),
                        ...
                        resultConvertNirvanaTX.first[k].getNVout()
                    });
                }
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // nirvana
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // nirvana
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // nirvana
            globalState->setRootUTXO(oldHashUTXORoot); // nirvana
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // nirvana
        uint32_t sizeBlockDGP = nirvanaDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Nirvana PoS we need to sync this after every block to ensure txdb is populated for
