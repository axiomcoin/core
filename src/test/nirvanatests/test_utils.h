...
extern std::unique_ptr<NirvanaState> globalState;
...
inline void initState(){
    ...
    const std::string dirNirvana = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<NirvanaState>(new NirvanaState(dev::u256(0), NirvanaState::openDB(dirNirvana, hashDB, dev::WithExisting::Trust), dirNirvana + "/nirvanaDB", dev::eth::BaseState::Empty));
...
inline dev::Address createNirvanaAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline NirvanaTransaction createNirvanaTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    NirvanaTransaction txEth;
    if(recipient == dev::Address()){
        txEth = NirvanaTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = NirvanaTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<NirvanaTransaction> txs){
