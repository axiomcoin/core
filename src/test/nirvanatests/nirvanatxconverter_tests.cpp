...
void checkResult(bool isCreation, std::vector<NirvanaTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    NirvanaTxConverter converter(transaction, NULL);
    ExtractNirvanaTX nirvanaTx;
    BOOST_CHECK(converter.extractionNirvanaTransactions(nirvanaTx));
    std::vector<NirvanaTransaction> result = nirvanaTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(nirvanatxconverter_tests, TestingSetup)
