...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createNirvanaTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createNirvanaTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createNirvanaTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createNirvanaTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    initState();
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    dev::eth::EVMSchedule schedule = nirvanaDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    dev::eth::EVMSchedule schedule = nirvanaDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    dev::eth::EVMSchedule schedule = nirvanaDGP.getGasSchedule(1400);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<NirvanaTransaction> txs;
    txs.push_back(createNirvanaTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createNirvanaTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    NirvanaDGP nirvanaDGP(globalState.get());
    dev::eth::EVMSchedule schedule = nirvanaDGP.getGasSchedule(502); // After initializing the tests, the height of the chain 502
...
//    initState();
    ...
    for(size_t i = 0; i < 1300; i++){
        dev::eth::EVMSchedule schedule = nirvanaDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    uint32_t blockSize = nirvanaDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    uint32_t blockSize = nirvanaDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<NirvanaTransaction> txs;
    txs.push_back(createNirvanaTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createNirvanaTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    NirvanaDGP nirvanaDGP(globalState.get());
    uint32_t blockSize = nirvanaDGP.getBlockSize(502);
...
//    initState();
    ...
    NirvanaDGP nirvanaDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint32_t blockSize = nirvanaDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    NirvanaDGP nirvanaDGP(globalState.get());
    uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<NirvanaTransaction> txs;
    txs.push_back(createNirvanaTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createNirvanaTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createNirvanaTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    NirvanaDGP nirvanaDGP(globalState.get());
    uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(502);
...
//    initState();
    ...
    NirvanaDGP nirvanaDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint64_t minGasPrice = nirvanaDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
