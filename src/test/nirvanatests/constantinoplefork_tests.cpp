...
//    initState();
    ...
    txs.push_back(createNirvanaTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createNirvanaTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createNirvanaAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<NirvanaTransaction> txsCall;
    txsCall.push_back(createNirvanaTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<NirvanaTransaction> txsMint;
    txsMint.push_back(createNirvanaTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<NirvanaTransaction> txsbalance;
    txsbalance.push_back(createNirvanaTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<NirvanaTransaction> txs;
    txs.push_back(createNirvanaTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createNirvanaAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<NirvanaTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createNirvanaTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
