...
#define paternVersion "nirvana-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

NirvanaVersionChecker::NirvanaVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

NirvanaVersionChecker::~NirvanaVersionChecker()
{

}

bool NirvanaVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> NirvanaVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(NIRVANA_RELEASES)));
    QEventLoop event;
...
Version NirvanaVersionChecker::getMaxReleaseVersion()
{
