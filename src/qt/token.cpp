...
bool ToHash160(const std::string& strNirvanaAddress, std::string& strHash160)
{
    CTxDestination nirvanaAddress = DecodeDestination(strNirvanaAddress);
    if(!IsValidDestination(nirvanaAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&nirvanaAddress);
    if(keyid){
...
bool ToNirvanaAddress(const std::string& strHash160, std::string& strNirvanaAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination nirvanaAddress = keyid;
    if(IsValidDestination(nirvanaAddress)){
        strNirvanaAddress = EncodeDestination(nirvanaAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToNirvanaAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToNirvanaAddress(tokenEvent.receiver, tokenEvent.receiver);
