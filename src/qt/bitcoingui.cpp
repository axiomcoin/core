...
#include <qt/nirvanaversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    nirvanaVersionChecker = new NirvanaVersionChecker(this);
#ifdef ENABLE_WALLET
    if(enableWallet) {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Nirvana address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nirvana: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Nirvana addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Nirvana addresses"));
    ...
    openAction->setStatusTip(tr("Open a nirvana: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Nirvana command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && nirvanaVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(NIRVANA_RELEASES, NIRVANA_RELEASES);
            QString message(tr("New version of Nirvana wallet is available on the Nirvana source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Nirvana network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
