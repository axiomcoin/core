 ...
 */
static const char* BITCOIN_PID_FILENAME = "nirvanad.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("nirvana-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nirvana-fnd/nirvana>";
    const std::string URL_WEBSITE = "<https://nirvana.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateNirvana");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path nirvanaStateDir = GetDataDir() / "stateNirvana";
                bool fStatus = fs::exists(nirvanaStateDir);
                const std::string dirNirvana(nirvanaStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsNirvanastate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<NirvanaState>(new NirvanaState(dev::u256(0), NirvanaState::openDB(dirNirvana, hashDB, dev::WithExisting::Trust), dirNirvana, existsNirvanastate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(nirvanaStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setNirvanaSchedule(nirvanaDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
